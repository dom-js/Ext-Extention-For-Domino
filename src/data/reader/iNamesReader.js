 ...
 *     }
 */
Ext.define('Domino.data.reader.iNamesReader', {
    ...
    getResponseData: function(response) {
        ...
        if (typeof responseText !== 'string') {
             return this.transferData(responseText);
           // return responseText;
        }
    ...
    transferData:function(obj){
       ...
       var data=[]
        //var data = obj.viewentry.entrydata
        //  console.log(obj)
        for(var i in obj.viewentry.entrydata)
            if(obj.viewentry.entrydata[i]["@name"]=="server"){ 
                var _obj  =obj.viewentry.entrydata[i].viewentries?(obj.viewentry.entrydata[i].viewentries.viewentry[0]||[]):[]; 
                if(_obj.entrydata)
                    for(var n in _obj.entrydata){
                        if(_obj.entrydata[n]["@name"]=="candidate"&&_obj.entrydata[n].viewentries&&_obj.entrydata[n].viewentries.viewentry)
                            for(var k in _obj.entrydata[n].viewentries.viewentry){
                                var _item = _obj.entrydata[n].viewentries.viewentry[k];
                                ...
                                for (var l in _item.entrydata){
                                    var __obj = _item.entrydata[l];
                                    var _n = __obj["@name"] ,_v = __obj.text[0]||"";
                                    item[_n] =_v
                                    switch(_n){
                                        case "internetAddress":
                                            //debugger
                                             var __n  = _v.match(/^[^@]+/);
                                             item["shortName"] = __n?(__n[0]||""):"";
                                            break;
                                        case "fullName":
                                             var __n = __obj.text[0].match(/CN=([^\/]+)\/O/);
                                             item["name"] = __n?(__n[1]||""):"";
                                ...
                                if (!item["name"]) {item["name"]=item["fullName"]};
